    ...
    if (element.hasOwnProperty(key)) {
      ...
      for (i = 0; i < nodes.length; ++i) {
        switch (key) {
          ...
          case options.textKey:
            xml.push(
              (options.indentText ? writeIndentation(options, depth, firstLine) : '') + writeText(nodes[i], options)
            )
            break
          case options.cdataKey:
            xml.push(
              (options.indentCdata ? writeIndentation(options, depth, firstLine) : '') + writeCdata(nodes[i], options)
            )
            break
          case options.doctypeKey:
          ...
          default:
            xml.push(
              writeIndentation(options, depth, firstLine) +
                writeElementCompact(nodes[i], key, options, depth, hasContentCompact(nodes[i], options))
            )
        }
...
export function js2xml(js, options) {
